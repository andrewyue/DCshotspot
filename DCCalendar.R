...
Raw_Data$hour = as.numeric(strftime(Raw_Data$Rdatetime, format="%H")) + as.numeric(strftime(Raw_Data$Rdatetime, format="%M"))/60
...
m1=ggplot(jan, aes(hour, fill=cut(..x..,c(7.5,17.25))))+geom_histogram(binwidth=.25, )+guides(fill=FALSE)+scale_x_continuous(breaks=c(0,6,12,18,24))+ggtitle("January")+coord_cartesian(ylim=c(0,200))+xlab("Time of Day")+ylab("Incidents")+annotate("text", x=12.5, y=30, family="Myriad Web Pro", label ="Daylight hours",size=4)+annotate("rect",xmin=7.5, xmax = 17.25, ymin=0, ymax=50,fill="orange",alpha=0.1)+theme(text=element_text(family="Myriad Web Pro", size=24))
